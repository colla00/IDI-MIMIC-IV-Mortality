...
Exclusion criteria:
  ...
  - Planned postoperative elective cardiac surgery admissions (n = 4,321)

Output: data/cohort.csv  (expected n = 26,153 before idi_features.py join)
...
# ── ICD codes for planned elective cardiac surgery ────────────────────────────
...
def select_cohort():
    ...
    # subject_id is excluded from the admissions subset to prevent
    # subject_id_x / subject_id_y column collision during the merge.
    df = icustays.merge(
    ...
    # Step 7: exclude admissions with missing discharge status
    ...
    # Step 8: exclude planned elective cardiac surgery admissions
    ...
    # Note: admissions with missing documentation timestamps (n = 1,847) are
    # excluded implicitly via inner join in idi_features.py. Stays with zero
    # valid chartevents timestamps in the first 24 h produce no IDI features
    # and are dropped at that stage, consistent with the manuscript exclusion.
    print(f"\n[NOTE] Missing documentation timestamps (n = 1,847) are excluded "
          f"via inner join in idi_features.py.")
