...
Metric: AUROC (95% CI via bootstrap, n=2000), ΔAUROC vs baseline,
        calibration slope and intercept per subgroup.

Subgroups with n < 10 are skipped entirely (insufficient for AUROC estimation).
Subgroups with 10 <= n < 50 are computed and saved with a small_sample_warning
flag to support equity transparency; results should be interpreted with caution
given wide confidence intervals. This matches the manuscript which reports these
subgroups with explicit small-sample caveats in Supplementary Table 1.

Requires: data/idi_features.csv must include a 'race' column, produced by
running cohort_selection.py (which pulls race from the admissions table).
"""
...
N_BOOT      = 2000
SEED        = 42
MIN_N_HARD  = 10   # skip entirely below this threshold
MIN_N_WARN  = 50   # flag with small_sample_warning below this threshold
...
def main():
    ...
    if "race" not in cohort.columns:
        raise RuntimeError(
            ...
            "Ensure you ran cohort_selection.py which saves the 'race' column."
        )
    ...
    for label, race_val in SUBGROUPS.items():
        ...
        if small_sample_warning:
            warnings.warn(
                ...
                f"interpreted with caution. CIs will be wide.",
    ...
    # Dynamic x-axis limits derived from data
    ...
    small_patch  = mpatches.Patch(color="darkorange",
                                  label="IDI-Enhanced AUROC (95% CI, n < 50 -- small sample)")
