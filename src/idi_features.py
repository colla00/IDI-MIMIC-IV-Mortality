...
Features (first 24 hours of ICU stay):
  ...
  Rhythm Regularity:
    ...
    idi_std_interevent_min  - SD of inter-event intervals
...
def extract_all_idi(cohort_df, ce):
    ...
    # Convert to minutes from admission (sorted, deduplicated)
    ce["minutes"] = ce["hours_from_admit"] * 60
...
def main():
    ...
    print(f"\nFinal dataset: {len(out):,} stays | "
          f"mortality {out['mortality'].mean()*100:.2f}%")
