...
FIXES APPLIED:
  v2 - ISSUE-9 : ddof=1 for sample std (was population std).
  v2 - ISSUE-10: window_hours now actually filters events to first N hours.
  v2 - MINOR-14: Removed unused `from scipy.stats import variation`.
  v2 - MINOR   : inter-event intervals computed in seconds -> float minutes.
  v3 - FIX-NEW : main() used out['mortality'] — column is 'hospital_mortality'.
                 Caused KeyError on every standalone run. Now corrected.
...
def extract_all_idi(cohort_df, ce):
    ...
    # FIX ISSUE-10: keep only first 24 hours from ICU admission
    ce["hours_from_admit"] = (ce["charttime"] - ce["intime"]).dt.total_seconds() / 3600
    ce = ce[(ce["hours_from_admit"] >= 0) & (ce["hours_from_admit"] <= 24)]

    # FIX MINOR: convert to float minutes (avoids timedelta64[m] integer truncation)
    ...
    for stay_id, grp in tqdm(ce.groupby("stay_id"), desc="Extracting IDI"):
        ...
        if len(ts) < MIN_EVENTS:
            continue                        # exclude stays with < 10 events
...
def main():
    ...
    # Merge features back with full cohort
    out = cohort.merge(feat_df, on="stay_id", how="inner")

    # FIX v3: was out['mortality'] — column is 'hospital_mortality' -> KeyError
