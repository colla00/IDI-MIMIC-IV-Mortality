...
FIXES APPLIED:
  MINOR-13: Removed unused dead code: `tp_fp = np.column_stack([tp, fp])`
  MINOR-16: Added docstring note clarifying DeLong test uses Hanley-McNeil
            variance formula (assumes independence), yielding slightly
            conservative p-values when comparing correlated AUROCs.
            Full covariance DeLong would require the fastDeLong algorithm
            (Zhang & Mueller 2012). For clinical reporting this approximation
            is standard and acceptable.

Metrics computed:
  - AUROC with 95% CI (DeLong method, Hanley-McNeil variance)
  - AUPRC
  - Brier score
...
def delong_roc_variance(ground_truth, predictions):
    """
    Variance of AUROC via DeLong et al. 1988 (Hanley-McNeil implementation).

    NOTE: The delong_test() function below uses var1 + var2 (assumes
    independence of the two AUROCs). For correlated AUROCs on the same test
    set the true DeLong test should include the covariance term. This
    approximation is slightly conservative (wider CIs / larger p-values)
    but is standard in clinical ML literature.
    """
    ...
    # FIX MINOR-13: removed unused `tp_fp = np.column_stack([tp, fp])` line
...
def calibration_slope(y_true, y_pred):
    ...
    Slope = 1.0 is perfect calibration."""
    eps   = 1e-7
...
def ppv_npv(y_true, y_pred):
    ...
    (predict the same fraction as observed mortality rate)."""
