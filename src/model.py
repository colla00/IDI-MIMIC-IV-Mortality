...
FIXES APPLIED:
  v2 - BUG-7   : Standardised outcome column to 'hospital_mortality'.
  v2 - ISSUE-8 : Leakage filter applied before model training.
  v2 - MINOR-13: Switched from pickle to joblib.
  v2 - MINOR   : Added os.makedirs guard for results/ directory.
  v3 - FIX-NEW : Added SimpleImputer(strategy='median') as first pipeline
                 step. Without it, StandardScaler crashes with ValueError
                 when any IDI feature contains NaN (possible for edge-case
                 stays with sparse chartevent data). Matches multinational
                 model.py which already had the imputer.
...
Features are z-score standardized on the training set (after median imputation).
...
from sklearn.impute import SimpleImputer          # FIX v3
...
# Standardised outcome column (BUG-7)
OUTCOME = "hospital_mortality"


# ── Leakage filter (ISSUE-8) ──────────────────────────────────────────────────
def apply_leakage_filter(df: pd.DataFrame,
    ...
    Prevents reverse-causal leakage: non-survivors have longer stays
    (median 71.3 h vs 53.1 h), so features tracking stay duration
    are post-outcome proxies, not true predictors.
...
def build_pipeline() -> Pipeline:
    """
    FIX v3: SimpleImputer added as step 1.
    Imputes NaN IDI features with training-set median before scaling.
    Prevents ValueError from StandardScaler when sparse stays produce NaNs.
    Pipeline order: impute -> scale -> classify.
    """
    return Pipeline([
        ("imputer", SimpleImputer(strategy="median")),   # FIX v3
...
def train_and_save(train: pd.DataFrame,
    ...
    # Save model via joblib (MINOR-13)
...
def main():
    ...
    # Apply leakage filter on TRAINING data only (ISSUE-8)
