...
FIXES APPLIED:
  BUG-7: Standardised outcome column name to 'hospital_mortality'
         (was 'mortality' — now consistent with cohort_selection.py v2
          and multinational repo).
  ISSUE-8: Leakage filter is now applied before model training, matching
           the methodology described in the README and paper.
  MINOR-13: Switched from pickle to joblib (more efficient for sklearn).
  MINOR: Added os.makedirs guard for results/ directory.
...
def train_and_save(train: pd.DataFrame,
    ...
    # Save model via joblib (FIX MINOR-13)
...
def main():
    ...
    # FIX ISSUE-8: apply leakage filter on TRAINING data only
